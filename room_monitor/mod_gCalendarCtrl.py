        ...
        for i0 in range(len(events)):
            ...
            try:
                try:
                    ...
                    strRTN += strTitle
                    #strRTN += strTitle.encode('utf-8')
                    strRTN += '\n'
                    '''
                    strTitle = str(event['summary'])
                    str_val += mChange_ord(strTitle)
                    strRTN += str_val
                    strRTN += '\n'
                    '''
...
def time_Arrenge2(str_time):
 ...
 #文字列をアスキーコードへ
 #**********************************************************
def mChange_ord(str_target_text):
    strRTN = ""
    iCnt = 0
    for s in str_target_text:
        strRTN += str(ord(s))
        iCnt += 1
        if iCnt < len(str_target_text):
            strRTN += ','
    return strRTN

#set_Calendar_API('primary', 2)